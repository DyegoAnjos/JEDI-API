...
class PartidasPerguntasRepository
{
    ...
    {
        // Parte 1: Top 10
        $consulta = '(SELECT jogador, MAX(pontuacao) AS pontuacao,(SUM(qtdAcertos)/(SUM(qtdAcertos)+SUM(qtdErros)))*100 AS percentualAcertos, 
        ...
        // Você precisa vincular o ID para cada vez que o token aparece ou usar nomes diferentes
        $stmt->bindParam(':idPartida', $idPartida);

        if ($jogador !== null) {
            // Como o token aparece de novo na subquery, vinculamos ao valor novamente
